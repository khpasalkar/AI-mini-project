...
library(caret)
#xgboost_conf = confusionMatrix(xgbpred, ts_label)

xgboost_conf=confusionMatrix(
  factor(xgbpred, levels = 1:2223),
  factor(ts_label, levels = 1:2223)
)

#confusionMatrix(xgbpred, ts_label)
xgboost_conf <<- xgboost_conf$table
xgboost_error <<- 100 -  100*sum(diag(xgboost_conf)) / sum(xgboost_conf)  
